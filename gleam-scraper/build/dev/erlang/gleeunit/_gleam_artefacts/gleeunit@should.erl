...
-spec equal(FLN, FLN) -> nil.
equal(A, B) ->
    gleeunit_ffi:should_equal(A, B).

-spec not_equal(FLO, FLO) -> nil.
not_equal(A, B) ->
    gleeunit_ffi:should_not_equal(A, B).

-spec be_ok({ok, FLP} | {error, any()}) -> FLP.
be_ok(A) ->
    gleeunit_ffi:should_be_ok(A).

-spec be_error({ok, any()} | {error, FLU}) -> FLU.
be_error(A) ->
    gleeunit_ffi:should_be_error(A).

-spec be_some(gleam@option:option(FLX)) -> FLX.
be_some(A) ->
    case A of
        {some, Value} ->
            Value;
